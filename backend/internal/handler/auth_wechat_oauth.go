...
func (h *AuthHandler) WeChatOAuthStart(c *gin.Context) {
	h.WeChatOAuthStartGateway(gatewayctx.FromGin(c))
}

func (h *AuthHandler) WeChatOAuthStartGateway(c gatewayctx.GatewayContext) {
	cfg, err := h.getWeChatOAuthConfigGateway(c.Request().Context(), c.QueryValue("mode"), c)
	if err != nil {
	...
	intent := normalizeWeChatOAuthIntent(c.QueryValue("intent"))
	secureCookie := isRequestHTTPSGateway(c)
	setOAuthCookieGateway(c, wechatOAuthStateCookieName, encodeCookieValue(state), wechatOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatOAuthRedirectCookieName, encodeCookieValue(redirectTo), wechatOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatOAuthIntentCookieName, encodeCookieValue(intent), wechatOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatOAuthModeCookieName, encodeCookieValue(cfg.mode), wechatOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	...
	if intent == oauthIntentBindCurrentUser {
		...
		setOAuthCookieGateway(c, wechatOAuthBindUserCookieName, encodeCookieValue(bindCookieValue), wechatOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	} else {
		clearOAuthCookieGateway(c, wechatOAuthBindUserCookieName, wechatOAuthCookiePath, secureCookie)
	}
...
func (h *AuthHandler) WeChatOAuthCallback(c *gin.Context) {
	h.WeChatOAuthCallbackGateway(gatewayctx.FromGin(c))
}

func (h *AuthHandler) WeChatOAuthCallbackGateway(c gatewayctx.GatewayContext) {
	...
	secureCookie := isRequestHTTPSGateway(c)
	defer func() {
		clearOAuthCookieGateway(c, wechatOAuthStateCookieName, wechatOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatOAuthRedirectCookieName, wechatOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatOAuthIntentCookieName, wechatOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatOAuthModeCookieName, wechatOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatOAuthBindUserCookieName, wechatOAuthCookiePath, secureCookie)
	}()

	expectedState, err := readCookieDecodedGateway(c, wechatOAuthStateCookieName)
	if err != nil || expectedState == "" || state != expectedState {
		redirectOAuthErrorGateway(c, frontendCallback, "invalid_state", "invalid oauth state", "")
		return
	}

	redirectTo, _ := readCookieDecodedGateway(c, wechatOAuthRedirectCookieName)
	...
	intent, _ := readCookieDecodedGateway(c, wechatOAuthIntentCookieName)
	mode, err := readCookieDecodedGateway(c, wechatOAuthModeCookieName)
	if err != nil || strings.TrimSpace(mode) == "" {
		redirectOAuthErrorGateway(c, frontendCallback, "invalid_state", "missing oauth mode", "")
		return
	}

	cfg, err := h.getWeChatOAuthConfigGateway(c.Request().Context(), mode, c)
	if err != nil {
		redirectOAuthErrorGateway(c, frontendCallback, "provider_error", infraerrors.Reason(err), infraerrors.Message(err))
		return
	}

	tokenResp, userInfo, err := fetchWeChatOAuthIdentity(c.Request().Context(), cfg, code)
	if err != nil {
		redirectOAuthErrorGateway(c, frontendCallback, "provider_error", "wechat_identity_fetch_failed", singleLine(err.Error()))
		return
	}
	...
	if providerSubject == "" {
		if cfg.requiresUnionID() {
			redirectOAuthErrorGateway(c, frontendCallback, "provider_error", "wechat_missing_unionid", "")
			return
		}
	...
	if normalizedIntent == wechatOAuthIntentBind {
		if err := h.createWeChatBindPendingSessionGateway(c, cfg, providerSubject, openid, redirectTo, browserSessionKey, upstreamClaims); err != nil {
			switch infraerrors.Code(err) {
			case http.StatusConflict:
				redirectOAuthErrorGateway(c, frontendCallback, "ownership_conflict", infraerrors.Reason(err), infraerrors.Message(err))
			case http.StatusUnauthorized, http.StatusForbidden:
				redirectOAuthErrorGateway(c, frontendCallback, "auth_required", infraerrors.Reason(err), infraerrors.Message(err))
			default:
	...
	if existingIdentityUser != nil {
		...
		if err := h.createWeChatPendingSessionGateway(c, normalizedIntent, providerSubject, existingIdentityUser.Email, redirectTo, browserSessionKey, upstreamClaims, nil, nil, &existingIdentityUser.ID); err != nil {
	...
	if h.isForceEmailOnThirdPartySignup(c.Request().Context()) {
		if err := h.createWeChatChoicePendingSessionGateway(
			c,
			identityRef,
			email,
			email,
...
func (h *AuthHandler) WeChatPaymentOAuthStart(c *gin.Context) {
	h.WeChatPaymentOAuthStartGateway(gatewayctx.FromGin(c))
}

func (h *AuthHandler) WeChatPaymentOAuthStartGateway(c gatewayctx.GatewayContext) {
	cfg, err := h.getWeChatOAuthConfigGateway(c.Request().Context(), "mp", c)
	if err != nil {
		response.ErrorFromContext(gatewayJSONResponder{ctx: c}, err)
		return
	}

	paymentType := normalizeWeChatPaymentType(c.QueryValue("payment_type"))
	if paymentType == "" {
		response.ErrorContext(gatewayJSONResponder{ctx: c}, http.StatusBadRequest, "Invalid payment type")
		return
	}
	...
	redirectTo := normalizeWeChatPaymentRedirectPath(sanitizeFrontendRedirectPath(c.QueryValue("redirect")))
	if redirectTo == "" {
		redirectTo = wechatPaymentOAuthDefaultTo
	}
	rawContext, err := encodeWeChatPaymentOAuthContext(wechatPaymentOAuthContext{
		PaymentType: paymentType,
		Amount:      strings.TrimSpace(c.QueryValue("amount")),
		OrderType:   strings.TrimSpace(c.QueryValue("order_type")),
		PlanID:      parseWeChatPaymentPlanID(c.QueryValue("plan_id")),
	})
	if err != nil {
		response.ErrorFromContext(gatewayJSONResponder{ctx: c}, infraerrors.InternalServer("OAUTH_CONTEXT_ENCODE_FAILED", "failed to encode oauth context").WithCause(err))
		return
	}

	scope := normalizeWeChatPaymentScope(c.QueryValue("scope"))
	secureCookie := isRequestHTTPSGateway(c)
	setOAuthCookieGateway(c, wechatPaymentOAuthStateName, encodeCookieValue(state), wechatPaymentOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatPaymentOAuthRedirect, encodeCookieValue(redirectTo), wechatPaymentOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatPaymentOAuthContextName, encodeCookieValue(rawContext), wechatPaymentOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)
	setOAuthCookieGateway(c, wechatPaymentOAuthScope, encodeCookieValue(scope), wechatPaymentOAuthCookiePath, wechatOAuthCookieMaxAgeSec, secureCookie)

	cfg.redirectURI = h.resolveWeChatPaymentOAuthCallbackURLGateway(c.Request().Context(), c)
	cfg.scope = scope
	authURL, err := buildWeChatAuthorizeURL(cfg, state)
	if err != nil {
		response.ErrorFromContext(gatewayJSONResponder{ctx: c}, infraerrors.InternalServer("OAUTH_BUILD_URL_FAILED", "failed to build oauth authorization url").WithCause(err))
...
func (h *AuthHandler) WeChatPaymentOAuthCallback(c *gin.Context) {
	h.WeChatPaymentOAuthCallbackGateway(gatewayctx.FromGin(c))
}

func (h *AuthHandler) WeChatPaymentOAuthCallbackGateway(c gatewayctx.GatewayContext) {
	...
	secureCookie := isRequestHTTPSGateway(c)
	defer func() {
		clearOAuthCookieGateway(c, wechatPaymentOAuthStateName, wechatPaymentOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatPaymentOAuthRedirect, wechatPaymentOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatPaymentOAuthContextName, wechatPaymentOAuthCookiePath, secureCookie)
		clearOAuthCookieGateway(c, wechatPaymentOAuthScope, wechatPaymentOAuthCookiePath, secureCookie)
	}()

	expectedState, err := readCookieDecodedGateway(c, wechatPaymentOAuthStateName)
	if err != nil || expectedState == "" || state != expectedState {
		redirectOAuthErrorGateway(c, frontendCallback, "invalid_state", "invalid oauth state", "")
		return
	}

	redirectTo, _ := readCookieDecodedGateway(c, wechatPaymentOAuthRedirect)
	redirectTo = normalizeWeChatPaymentRedirectPath(sanitizeFrontendRedirectPath(redirectTo))
	if redirectTo == "" {
		redirectTo = wechatPaymentOAuthDefaultTo
	}

	rawContext, _ := readCookieDecodedGateway(c, wechatPaymentOAuthContextName)
	paymentContext, err := decodeWeChatPaymentOAuthContext(rawContext)
	if err != nil {
		redirectOAuthErrorGateway(c, frontendCallback, "invalid_context", "invalid oauth context", "")
		return
	}
	...
	scope, _ := readCookieDecodedGateway(c, wechatPaymentOAuthScope)
	...
	if err != nil {
		redirectOAuthErrorGateway(c, frontendCallback, "token_exchange_failed", "failed to exchange oauth code", err.Error())
		return
	}

	openid := strings.TrimSpace(tokenResp.OpenID)
	if openid == "" {
		redirectOAuthErrorGateway(c, frontendCallback, "missing_openid", "missing openid", "")
		return
	}
	...
	if err != nil {
		redirectOAuthErrorGateway(c, frontendCallback, "invalid_context", "failed to encode payment resume context", "")
		return
	}
...
func (h *AuthHandler) CompleteWeChatOAuthRegistration(c *gin.Context) {
	h.CompleteWeChatOAuthRegistrationGateway(gatewayctx.FromGin(c))
}

func (h *AuthHandler) CompleteWeChatOAuthRegistrationGateway(c gatewayctx.GatewayContext) {
...
func (h *AuthHandler) createWeChatPendingSessionGateway(
...
func (h *AuthHandler) createWeChatChoicePendingSessionGateway(
	c gatewayctx.GatewayContext,
	identity service.PendingAuthIdentityKey,
	suggestedEmail string,
	resolvedEmail string,
...
func (h *AuthHandler) createWeChatBindPendingSessionGateway(
	c gatewayctx.GatewayContext,
	cfg wechatOAuthConfig,
	providerSubject string,
	channelSubject string,
	...
	currentUser, err := h.readOAuthBindTargetUserGateway(c, wechatOAuthBindUserCookieName)
	if err != nil {
		return err
	}
	if err := h.ensureWeChatBindOwnership(c.Request().Context(), currentUser.ID, providerSubject, cfg, channelSubject); err != nil {
	...
	return h.createWeChatPendingSessionGateway(
		c,
		wechatOAuthIntentBind,
		providerSubject,
		currentUser.Email,
...
func (h *AuthHandler) readOAuthBindTargetUserGateway(c gatewayctx.GatewayContext, cookieName string) (*dbent.User, error) {
	client := h.entClient()
	if client == nil {
		return nil, infraerrors.ServiceUnavailable("PENDING_AUTH_NOT_READY", "pending auth service is not ready")
	}
	userID, err := h.readOAuthBindUserIDFromCookieGateway(c, cookieName)
	if err != nil {
		return nil, infraerrors.Unauthorized("AUTH_REQUIRED", "current user is required to bind wechat account")
	}
	userEntity, err := client.User.Get(c.Request().Context(), userID)
...
func (h *AuthHandler) getWeChatOAuthConfigGateway(ctx context.Context, rawMode string, c gatewayctx.GatewayContext) (wechatOAuthConfig, error) {
	mode, err := resolveWeChatOAuthModeGateway(rawMode, c)
	if err != nil {
		return wechatOAuthConfig{}, err
	}
	...
	cfg := wechatOAuthConfig{
		...
		appID:            strings.TrimSpace(effective.AppIDForMode(mode)),
		appSecret:        strings.TrimSpace(effective.AppSecretForMode(mode)),
		redirectURI:      firstNonEmpty(strings.TrimSpace(effective.RedirectURL), resolveWeChatOAuthAbsoluteURLGateway(apiBaseURL, c, "/api/v1/auth/oauth/wechat/callback")),
		frontendCallback: firstNonEmpty(strings.TrimSpace(effective.FrontendRedirectURL), wechatOAuthDefaultFrontendCB),
		scope:            effective.ScopeForMode(mode),
		openEnabled:      effective.OpenEnabled,
		mpEnabled:        effective.MPEnabled,
...
func resolveWeChatOAuthModeGateway(rawMode string, c gatewayctx.GatewayContext) (string, error) {
	mode := strings.ToLower(strings.TrimSpace(rawMode))
	if mode == "" {
		if isWeChatBrowserRequestGateway(c) {
			return "mp", nil
		}
		return "open", nil
	}
...
func isWeChatBrowserRequestGateway(c gatewayctx.GatewayContext) bool {
	if c == nil {
		return false
	}
	return strings.Contains(strings.ToLower(strings.TrimSpace(c.HeaderValue("User-Agent"))), "micromessenger")
}
...
func resolveWeChatOAuthAbsoluteURLGateway(apiBaseURL string, c gatewayctx.GatewayContext, callbackPath string) string {
	callbackPath = strings.TrimSpace(callbackPath)
	if callbackPath == "" {
		return ""
	}
	...
	scheme := "http"
	if isRequestHTTPSGateway(c) {
		scheme = "https"
	}
	host := strings.TrimSpace(c.Request().Host)
	if forwardedHost := strings.TrimSpace(c.HeaderValue("X-Forwarded-Host")); forwardedHost != "" {
		host = forwardedHost
	}
	if host == "" {
		return ""
...
func (h *AuthHandler) resolveWeChatPaymentOAuthCallbackURLGateway(ctx context.Context, c gatewayctx.GatewayContext) string {
	apiBaseURL := ""
	if h != nil && h.settingSvc != nil {
		if settings, err := h.settingSvc.GetAllSettings(ctx); err == nil && settings != nil {
			apiBaseURL = strings.TrimSpace(settings.APIBaseURL)
	...
	return resolveWeChatOAuthAbsoluteURLGateway(apiBaseURL, c, "/api/v1/auth/oauth/wechat/payment/callback")
}
